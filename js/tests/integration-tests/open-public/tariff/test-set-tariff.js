...
QUnit.module("setting tariff");

  function testSetTariff(setupNum) {
    switch (setupNum) {
    case 1:
      var data = getLinearGraph1();
      data.setWp(0.35);
      data.setTariffAmount(0.45);
      return data;
    default:
  ...
  QUnit.test("correct tariff amount", function(assert) {
    var data = testSetTariff(1);
    assert.deepEqual(data.tariffAmount, 0.45);
  });
  ...
  QUnit.test("correct tariff revenue", function(assert) {
    ...
    assert.deepEqual(data.getTariffRevenue().getForTesting(), 3);
  });